...
DISTANCE_THRESHOLD_FT = 10.0  # Objects within this many feet trigger warnings
...
def process_video(video_path, output_path=None):
    ...
    # Initialize video writer for side-by-side output
    out = None
    if output_path:
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        combined_width = width * 2  # Since we're stacking frames horizontally
        out = cv2.VideoWriter(output_path,
                            cv2.VideoWriter_fourcc(*'mp4v'),
                            fps,
                            (combined_width, height))  # Updated dimensions
    ...
    try:
        while True:
            ...
            if out:
                # Combine original and depth colormap side by side
                combined_frame = np.hstack((display_frame, depth_colormap))
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(OUTPUT_DIR, f"{video_name}_processed_no_green.mp4")
