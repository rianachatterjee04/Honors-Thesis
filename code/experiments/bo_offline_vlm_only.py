#!/usr/bin/env python3
"""
GenAssist — Phase 2 Offline Shader Bayesian Optimization (MOONDREAM BBOX)
==========================================================================
Replaces YOLO with Moondream for bounding box detection + confidence scoring.

Moondream detect returns normalized boxes (x_min, y_min, x_max, y_max).
Since Moondream has no per-box confidence score, we derive a pseudo-confidence:
    pseudo_conf = clamp(num_detections / MAX_EXPECTED_DETECTIONS, 0, 1)
This gives the BO threshold the same meaning as before:
    - High detections → high pseudo_conf → above threshold → YOLO_ONLY (no VLM description)
    - Low/uncertain detections → low pseudo_conf → below threshold → trigger VLM description

Two modes for Moondream:
    --moondream_mode local   → calls your existing localhost VLM server (add /detect endpoint)
    --moondream_mode api     → calls api.moondream.ai (needs MOONDREAM_API_KEY env var)

Your local VLM server /detect endpoint should accept:
    POST /detect
    { "image": "<base64 jpg>", "objects": ["person", "chair", "obstacle", ...] }
    → { "detections": [{"label": "person", "x_min": 0.1, "y_min": 0.2, "x_max": 0.4, "y_max": 0.8}, ...] }

Usage:
    python3 bo_offline_moondream.py --condition amd --video results/VR/AMD/trial_x_0.1_raw.mp4

    # Use Moondream Cloud API instead of local server:
    python3 bo_offline_moondream.py --condition amd --video results/VR/AMD/trial_x_0.1_raw.mp4 --moondream_mode api

    # Manual threshold:
    python3 bo_offline_moondream.py --condition amd --video results/VR/AMD/trial_x_0.1_raw.mp4 --threshold 0.35

    # No live window:
    python3 bo_offline_moondream.py --condition amd --video results/VR/AMD/trial_x_0.1_raw.mp4 --no_display
"""
...
VLM_ENDPOINT   = "http://localhost:5000/vlm"      # text description endpoint (Granite etc.)
DETECT_ENDPOINT = "http://localhost:5000/detect"  # Moondream detect endpoint on your local server

# Objects Moondream will look for — edit this list to match your environment
DETECT_LABELS = ["person", "chair", "obstacle"]
MAX_EXPECTED_DETECTIONS = 4  # pseudo-conf = detections / this, clamped to 1.0
DETECT_TIMEOUT = 90.0        # seconds per request — increase if Moondream is slow
...
def frame_to_b64(frame: np.ndarray, size=(320, 240)) -> str:
    small = cv2.resize(frame, size)
    _, buf = cv2.imencode('.jpg', small, [int(cv2.IMWRITE_JPEG_QUALITY), 75])
    return base64.b64encode(buf).decode('utf-8')


def detect_local(frame: np.ndarray):
    """
    Call the local VLM server /detect (OWLv2 — class-agnostic).
    Always returns (detections: list, pseudo_conf: float) — never raises.
    """
    try:
        img_b64 = frame_to_b64(frame)
        payload = {"image": img_b64}
        resp = requests.post(DETECT_ENDPOINT, json=payload, timeout=60.0)
        resp.raise_for_status()
        data = resp.json()
        detections  = data.get("detections", []) or []
        pseudo_conf = float(data.get("pseudo_conf", 0.0))
        return detections, pseudo_conf
    except Exception as e:
        print(f"[detect_local] error: {e}")
        return [], 0.0


def detect_api(frame: np.ndarray, api_key: str) -> list:
    """
    Call Moondream Cloud API for each label.
    Returns unified list of detections across all labels.
    """
    import moondream as md
    # Convert frame to PIL for moondream SDK
    ...
    for label in DETECT_LABELS:
        try:
            result = model.detect(pil_img, label)
            ...
            print(f"[Moondream API] detect '{label}' failed: {e}")
    return detections


def compute_pseudo_conf(detections: list) -> float:
    """
    Derive a scalar confidence score from Moondream detections.
    Logic: more detections + larger boxes = higher scene complexity = higher pseudo_conf.
    This preserves the original BO threshold semantics:
        pseudo_conf < threshold → uncertain scene → trigger VLM description
        pseudo_conf >= threshold → scene clear enough → skip VLM
    """
    if not detections:
        return 0.0
    # Weight by average box area (larger boxes = more confident coverage)
    areas = [
    ...
    n = len(detections)
    # Combine count and area into a 0-1 score
    count_score = min(n / MAX_EXPECTED_DETECTIONS, 1.0)
    area_score = min(avg_area * 4.0, 1.0)  # area of 0.25 = fully confident
    return float(np.clip((count_score + area_score) / 2.0, 0.0, 1.0))


def draw_moondream_boxes(frame: np.ndarray, detections: list) -> np.ndarray:
    """Draw Moondream bounding boxes on frame (normalized coords → pixel coords)."""
    out = frame.copy()
    h, w = out.shape[:2]
...
def draw_hud(frame: np.ndarray, threshold: float, pseudo_conf: float,
    ...
    conf_color = (0, 255, 0) if pseudo_conf >= threshold else (0, 80, 255)
    cv2.putText(out, f"thresh={threshold:.3f}  conf={pseudo_conf:.3f}",
                (w - 280, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.52, conf_color, 1, cv2.LINE_AA)
...
# ── VLM description worker (Granite / your existing endpoint) ─────────────────
...
class VLMWorker:
    ...
    def _worker(self, frame, on_done):
        ...
        try:
            ...
            payload = {
                "prompt": (
                    "You are an assistive vision system for someone with low vision. "
                    "The bounding boxes on screen show detected objects. "
                    "Describe any hazards, obstacles, or navigation guidance based on what you see. "
                    "Ignore visual distortion — it is a simulated impairment. "
                    "Answer in one or two clear, actionable sentences."
                ),
                "image": img_b64,
            }
...
def run_trial(condition: str, threshold: float, output_dir: str,
              trial_seed: int, moondream_mode: str,
              moondream_api_key: str = None, show_display: bool = True):
    ...
    writer_csv.writerow(["frame_idx", "source_env", "source_file",
                         "moondream_detections", "detected_classes",
                         "pseudo_conf", "status",
    ...
    def on_vlm_done(desc, lat, frame_idx, env, src):
        ...
        writer_csv.writerow([frame_idx, env, src, "", "VLM_RESPONSE_RECEIVED",
                              "", desc, f"{lat:.1f}", f"{usage:.4f}"])
        csvf.flush()
    ...
    for video_info in SOURCE_VIDEOS:
        ...
        print(f"\n  --> Analyzing: {src_name} | env={env_label}")
        ...
        while True:
            ...
            # ── Moondream detection (replaces YOLO) ───────────────────────
            detections = []
            pseudo_conf = 0.0
            detected_classes = "none"
            try:
                if moondream_mode == "api":
                    detections = detect_api(filtered, moondream_api_key)
                    pseudo_conf = compute_pseudo_conf(detections)
                else:
                    # OWLv2 server returns pseudo_conf directly
                    detections, pseudo_conf = detect_local(filtered)

                detected_classes = ";".join(set(d["label"] for d in detections)) if detections else "none"
            ...
            # ── VLM trigger ───────────────────────────────────────────────
            # Primary: Moondream pseudo_conf gates Granite (like YOLO did)
            # Fallback: if Moondream returns nothing, use frame-interval trigger
            #           so Granite always gets called and trials don't stall
            FALLBACK_INTERVAL = max(1, int(threshold * 30))
            moondream_working = pseudo_conf > 0.0 or len(detections) > 0
            interval_trigger = (frame_idx % FALLBACK_INTERVAL == 0)
            conf_trigger = 0.01 < pseudo_conf < threshold

            if (conf_trigger or (not moondream_working and interval_trigger)) and not vlm.busy:
                ...
                status = "MOONDREAM_ONLY"

            # CSV log
            writer_csv.writerow([
                frame_idx, env_label, src_name,
                len(detections), detected_classes,
                f"{pseudo_conf:.4f}", status,
                "PENDING..." if status == "VLM_TRIGGERED" else "N/A",
                "0", f"{usage_ratio:.4f}"
            ])
            csvf.flush()

            # Draw Moondream boxes on the filtered frame
            annotated = draw_moondream_boxes(filtered, detections)

            # Build HUD
            display = draw_hud(
                annotated, threshold, pseudo_conf, status,
            ...
            if show_display:
                ...
                if status == "VLM_TRIGGERED":
                    ...
                    while vlm.busy:
                        ...
                        elapsed = time.time() - freeze_start
                    ...
                    if not goto_end and current_vlm_text:
                        response_frame = draw_hud(
                            annotated, threshold, pseudo_conf, "MOONDREAM_ONLY",
                            current_vlm_text, condition,
                            vlm.total_calls, total_steps
                        )
...
def main():
    parser = argparse.ArgumentParser(
        description="GenAssist Phase 2 — Moondream bbox + VLM description BO"
    )
    parser.add_argument('--condition', required=True,
                        choices=['amd', 'glaucoma', 'rp', 'dr'])
    parser.add_argument('--bo_db', type=str, default=None)
    parser.add_argument('--threshold', type=float, default=None,
                        help="Manual threshold for pseudo_conf gate (0-1)")
    ...
    parser.add_argument('--moondream_mode', choices=['local', 'api'], default='local',
                        help="'local' = POST to localhost:5000/detect, 'api' = Moondream Cloud")
    parser.add_argument('--moondream_api_key', type=str, default=None,
    ...
    moondream_api_key = args.moondream_api_key or os.environ.get("MOONDREAM_API_KEY")
    if args.moondream_mode == "api" and not moondream_api_key:
        print("[ERROR] --moondream_mode api requires --moondream_api_key or MOONDREAM_API_KEY env var")
        return
    ...
    print(f"\n{'='*55}")
    print(f"  PHASE 2 TRIAL — MOONDREAM BBOX + VLM DESCRIPTION")
    print(f"  Condition      : {CONDITION_LABELS[condition]}")
    print(f"  Threshold      : {threshold:.4f}")
    print(f"  Moondream mode : {args.moondream_mode}")
    print(f"  Detect labels  : {DETECT_LABELS}")
    ...
    if args.video:
        ...
        if not discovered:
            print(f"[ERROR] No videos found in {args.results_dir}")
            return
        SOURCE_VIDEOS.clear()
        ...
        print(f"[Video] {len(SOURCE_VIDEOS)} video(s) discovered")
    else:
        print("[ERROR] Specify --video or --results_dir")
        return

    avg_latency, usage_ratio = run_trial(
        condition, threshold, output_dir, trial_seed,
        args.moondream_mode, moondream_api_key, show_display
    )
    ...
    print()
    print("Score the guidance quality:")
    print("  1  = VLM triggered too rarely (threshold too high) or too often (too low)")
    ...
    print(f"     Run: python3 bo_offline_moondream.py --condition {condition} --video <path>")
    print()
