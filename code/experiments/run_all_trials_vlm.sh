#!/bin/sh
# ─────────────────────────────────────────────────────────────────────────────
# run_all_trials_vlm.sh  —  macOS compatible (sh/zsh safe)
# VLM-only version: no YOLO. Threshold controls VLM call frequency.
# Runs BO trials for each condition, stopping when any threshold has been
# tried 3 times (convergence signal).
# - Creates a brand new bo_history_offline.csv inside each _Offline_VLM folder
# - Shows live cv2 window for each video
# - Stores annotated video + CSV in <CONDITION>_Offline_VLM/
#
# Usage (run from your in-lab/ directory):
#   ./in_line_test/run_all_trials_vlm.sh           # all 4 conditions
#   ./in_line_test/run_all_trials_vlm.sh AMD       # one condition only
# ─────────────────────────────────────────────────────────────────────────────

VR_DIR="results/VR"
SCRIPT="in_line_test/bo_offline_vlm_only.py"
...
for FOLDER in $FOLDERS; do
  ...
  OUTPUT_DIR="${VR_DIR}/${FOLDER}_Offline_VLM"
