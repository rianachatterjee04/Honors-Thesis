...
def get_next_threshold(db_path="bo_history.csv"):
    """
    Calculates the next OWLv2 pseudo_conf threshold using Bayesian Optimization.
...
# --- OWLv2 DETECTION VIA FLASK SERVER ---

def call_owlv2_detect(frame, detect_endpoint, logger=None):
    """
    Sends a frame to the Flask /detect endpoint and returns
    (pseudo_conf, detections). Returns (None, []) on failure.

    pseudo_conf is the scalar 0-1 score from OWLv2:
      - High  → scene well-covered, skip Granite VLM
      - Low   → uncertain scene, trigger Granite VLM
    """
    try:
        ...
        if response.status_code == 200:
            ...
            return pseudo_conf, detections
        else:
            if logger:
                logger.warn(f"OWLv2 /detect returned status {response.status_code}")
            return None, []

    except Exception as e:
        if logger:
            logger.warn(f"OWLv2 /detect request failed: {e}")
        return None, []


def draw_owlv2_boxes(frame, detections):
    """
    Draws OWLv2 bounding boxes onto the frame in-place.
    Detections use normalized coords (0-1); we scale to frame size.
    """
    h, w = frame.shape[:2]
...
class Go2WalkForward(Node):
    def __init__(self, threshold):
        ...
        # OWLv2 and VLM endpoints — both served from the same Flask process
        self.detect_endpoint = "http://localhost:5000/detect"
        ...
        # Last OWLv2 detections for annotation — updated each pose callback
        ...
        self.csv_writer.writerow([
            "stamp_sec", "x", "y", "z",
            "owl_pseudo_conf", "num_detections",
            "status", "vlm_response", "latency_ms", "usage_ratio"
        ])
    ...
    def vlm_request_worker(self, frame, stamp, pose_x):
        """Worker thread: sends frame to Granite VLM and logs result once received."""
        ...
        try:
            ...
            payload = {
                "prompt": "Identify obstacles and hazards that a visually impaired person should look out for.",
                "image": img_base64
            }
    ...
    def pose_callback(self, msg: PoseStamped):
        ...
        # --- OWLv2 detection (replaces YOLO) ---
        pseudo_conf, detections = call_owlv2_detect(
            frame, self.detect_endpoint, logger=self.get_logger()
        )
        self.latest_detections = detections

        # If the /detect call failed entirely, treat as uncertain → trigger VLM
        if pseudo_conf is None:
            pseudo_conf = 0.0

        num_detections = len(detections)

        # Gate logic: low pseudo_conf = uncertain scene → call Granite VLM
        # (mirrors original YOLO logic: low max_conf triggered VLM)
        if pseudo_conf < self.conf_threshold and not self.vlm_busy:
            self.vlm_busy = True
            threading.Thread(
                target=self.vlm_request_worker,
        ...
        # Annotate frame with OWLv2 boxes
        annotated_frame = draw_owlv2_boxes(frame.copy(), detections)
        self.video_writer.write(annotated_frame)
        self.raw_video_writer.write(frame)  # unannotated — for offline BO trials

        p_x = self.total_vlm_calls / self.total_steps if self.total_steps > 0 else 0.0
        self.csv_writer.writerow([
            msg.header.stamp.sec, msg.pose.position.x, 0.0, 0.0,
            round(pseudo_conf, 4), num_detections,
            status, "PENDING...", 0.0, p_x
        ])
        self.csv_file.flush()
